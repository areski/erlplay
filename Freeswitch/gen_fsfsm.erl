...
dispatch({timeout, _Ref, {'$gen_event', Event}}, Mod, StateName, StateData) ->
    Mod:StateName(Event, StateData);
dispatch({E, {event, [UUID | Data]}}, Mod, StateName, StateData) ->
	Name = proplists:get_value(<<"Event-Name">>, Data),
	case Name of
		<<"CHANNEL_HANGUP_COMPLETE">> ->
			Mod:handle_event({channel_hangup_event, binary_to_list(UUID), Data}, StateName, StateData);
		<<"CHANNEL_DESTROY">> ->
			Mod:handle_event({channel_destroy_event, binary_to_list(UUID), Data}, StateName, StateData);
		_ ->
			Mod:StateName({E, Name, binary_to_list(UUID), Data}, StateData)
	end;
